...
- Under matched linear SVM with subject-level leave-one-out, the density-matrix feature space reached LOSO AUC [0.785]{.num-chip} [subject-bootstrap 95% CI 0.58–0.95] against the matched classical comparator (paired DeLong [p = 0.001]{.num-chip}); under matched Random Forest the gap is not statistically detectable at this N (ΔAUC [0.097]{.num-chip}, [p = 0.51]{.num-chip}). No cell clears chance under per-fold permutation at $N = 28$ (best [p = 0.078]{.num-chip}). Per-fold means from the original 8-classifier sweep are retained in [Figure 5](sections/04-density-matrix.qmd#auc-comparison-chart).
